...
def main():
    ...
    SAVE_DIR = os.path.join(PROJECT_ROOT, "model_outcomes", "checkpoints_gpr_fishnet_final")
    BEST_CHECKPOINT_FILENAME = "best_gpr_fishnet.pth"

    ENV_VARS = ["thetao", "chl", "uo", "vo", "so", "zos", "o2"]
    SEQ_LEN, PRED_LEN, ROLLOUT_2024_HORIZON, ROLLOUT_START_INDEX = 12, 1, 12, 144
    BATCH_SIZE, ACCUMULATION_STEPS = 2, 4
    EPOCHS = 100
    LEARNING_RATE, WEIGHT_DECAY = 1e-4, 1e-5
    SEEDS, TRAIN_LOSS, HOTSPOT_THRESHOLD = DEFAULT_SEEDS, "mse", 0.2755
    EXTRA_HOTSPOT_THRESHOLDS = [0.3175]
    ...
    for seed in SEEDS:
        ...
        model = GPRFishNet(in_chans=8, hidden_dim=64, img_size=(64, 96), num_layers=2).to(device)
        ...
        print(f"Starting {MODEL_NAME} training with seed={seed}...")
        ...
        print(f"\n{MODEL_NAME} final one-step test metrics:")
        for k, v in one_step_metrics.items():
            print(f"  {k}: {v:.6f}")
        print(f"\n{MODEL_NAME} final 2024 rollout metrics:")
