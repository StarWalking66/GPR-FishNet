...
from baseline.models.convlstm_model import ConvLSTM, ConvLSTMBaseline as ModelConvLSTMBaseline
...
class ConvLSTMBaseline(nn.Module):
    ...
    def __init__(
        ...
        self.predictor = nn.Conv2d(
            ...
            out_channels=self.pred_len * self.out_channels, 
            kernel_size=1,  
            padding=0
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.predictor(last_h) # [B, 1, H, W]
        return out.unsqueeze(1)

# =====================================================================
def evaluate_convlstm(model, loader, device, loss_name, mask_tensor=None):
    model.eval()
    total_loss = 0.0
    with torch.no_grad():
...
def rollout_predict_2024_convlstm(model, data_dir, env_vars, device, seq_len, horizon, start_idx, mask_np):
    model.eval()
    ...
    with torch.no_grad():
        for step in range(horizon):
            current_input = np.concatenate([hist_env, hist_ais], axis=1) # [12, 8, H, W]
            input_tensor = torch.from_numpy(current_input[None]).float().to(device) # [1, 12, 8, H, W]
            
            pred = apply_output_clamp(model(input_tensor))
            pred_step = pred[0, 0].cpu().numpy() # [1, H, W]
            
            if mask_np is not None: 
                pred_step[0, mask_np == 0] = 0.0
            
            preds.append(pred_step)
            targets.append(ais_full[start_idx + step]) # [1, H, W]
...
def main():
    ...
    SAVE_DIR = "./model_outcomes/checkpoints_convlstm_baseline"
    ...
    for seed in SEEDS:
        ...
        best_model_path = os.path.join(run_save_dir, "best_convlstm.pth")
        print(f"Starting ConvLSTM baseline training with seed={seed}...")
        for epoch in range(EPOCHS):
            ...
            avg_val_loss = evaluate_convlstm(model, val_loader, device, TRAIN_LOSS, mask_tensor)
            ...
            print(f"\nSeed {seed} | Epoch {epoch + 1:03d} | Train: {avg_train_loss:.6f} | Val: {avg_val_loss:.6f} | LR: {current_lr:.2e}")
            ...
            if avg_val_loss < best_val_loss:
                ...
                print("ConvLSTM best model updated.")
            else:
                no_improve_epochs += 1
        ...
        print("\nRunning ConvLSTM one-step test inference...")
        model.eval()
        all_preds, all_targets, all_inputs = [], [], []
        with torch.no_grad():
            for inputs, targets in tqdm(test_loader, desc="ConvLSTM Test Inference"):
        ...
        if float(all_preds.std()) < 1e-6:
            print("Warning: ConvLSTM predictions appear collapsed (near-constant output).")

        np.save(os.path.join(run_save_dir, "test_one_step_preds.npy"), all_preds)
        np.save(os.path.join(run_save_dir, "test_one_step_targets.npy"), all_targets)
        np.save(os.path.join(run_save_dir, "test_one_step_inputs.npy"), all_inputs)

        one_step_metrics = compute_metrics(all_preds, all_targets, mask_tensor, HOTSPOT_THRESHOLD)
        save_json(one_step_metrics, os.path.join(run_save_dir, "test_one_step_metrics.json"))
        save_example_predictions(run_save_dir, all_preds, all_targets, prefix="test_one_step_example")
        print("\nRunning ConvLSTM 2024 autoregressive rollout...")
        mask_np = mask_tensor.detach().cpu().numpy() if mask_tensor is not None else None
        rollout_preds, rollout_targets = rollout_predict_2024_convlstm(
            model, DATA_DIR, ENV_VARS, device, SEQ_LEN, ROLLOUT_2024_HORIZON, ROLLOUT_START_INDEX, mask_np
        )

        np.save(os.path.join(run_save_dir, "rollout_2024_preds.npy"), rollout_preds)
        np.save(os.path.join(run_save_dir, "rollout_2024_targets.npy"), rollout_targets)

        rollout_metrics = compute_metrics(rollout_preds, rollout_targets, mask_tensor, HOTSPOT_THRESHOLD)
        save_json(rollout_metrics, os.path.join(run_save_dir, "rollout_2024_metrics.json"))
        ...
        print("\nConvLSTM baseline final one-step test metrics:")
        for k, v in one_step_metrics.items(): print(f"  {k}: {v:.6f}")
        print("\nConvLSTM baseline final 2024 rollout metrics:")
    ...
    finalize_multi_seed_experiment(
        ...
        best_checkpoint_filename="best_convlstm.pth",
    )
