...
def main():
    data_dir = os.path.join(PROJECT_ROOT, "data", "ST_FishNet_Features")
    mask_path = os.path.join(data_dir, "all_vars_train_mask_intersection.npy")
    save_dir = os.path.join(PROJECT_ROOT, "model_outcomes", "checkpoints_seacast_baseline")

    env_vars = ENV_VARS
    seq_len, pred_len = SEQ_LEN, PRED_LEN
    rollout_horizon, rollout_start_index = ROLLOUT_2024_HORIZON, ROLLOUT_START_INDEX
    batch_size, accumulation_steps = BATCH_SIZE, ACCUMULATION_STEPS
    epochs = EPOCHS
    learning_rate, weight_decay = LEARNING_RATE, WEIGHT_DECAY
    seeds, train_loss, hotspot_threshold = DEFAULT_SEEDS, TRAIN_LOSS, HOTSPOT_THRESHOLD
    hidden_dim, graph_blocks, coarse_factor = 64, 2, 4

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    use_amp = device.type == "cuda"
    os.makedirs(save_dir, exist_ok=True)

    mask_np = np.load(mask_path).astype(np.float32) if os.path.exists(mask_path) else None
    mask_tensor = torch.from_numpy(mask_np).to(device) if mask_np is not None else None
    ...
    for seed in seeds:
        ...
        best_model_path = os.path.join(run_save_dir, "best_seacast.pth")

        print(f"Starting SeaCast baseline training with seed={seed}...")
        ...
        rollout_mask_np = mask_tensor.detach().cpu().numpy() if mask_tensor is not None else None
        ...
        print("\nSeaCast baseline final one-step test metrics:")
        for key, value in one_step_metrics.items():
            print(f"  {key}: {value:.6f}")
        print("\nSeaCast baseline final 2024 rollout metrics:")
    ...
    finalize_multi_seed_experiment(
        ...
        best_checkpoint_filename="best_seacast.pth",
    )
