...
from baseline.models.exprecast_model import ExPreCast  # 淇敼瀵煎叆璺緞
...
from baseline.train.native_losses import ExPreCastFACL
...
def main():
    ...
    # 鏇存敼淇濆瓨璺緞涓?exprecast 涓撳睘鐩綍
    SAVE_DIR = "./model_outcomes/checkpoints_exprecast_final"
    ...
    for seed in SEEDS:
        ...
        model = ExPreCast(in_chans=8, hidden_dim=64, img_size=(64, 96), num_layers=2).to(device)
        train_criterion = ExPreCastFACL(total_steps=max(1, EPOCHS * len(train_loader)))
        ...
        best_model_path = os.path.join(run_save_dir, "best_exprecast.pth")

        print(f"Starting ExPreCast training with seed={seed}...")
        for epoch in range(EPOCHS):
            ...
            for i, (inputs, targets) in enumerate(pbar):
                ...
                with torch.amp.autocast(device_type='cuda', enabled=use_amp):
                    ...
                    aux_loss = train_criterion(preds, targets)
                    loss = main_loss + EXPRECAST_FACL_WEIGHT * aux_loss
        ...
        print("\nExPreCast final one-step test metrics:")
        for k, v in one_step_metrics.items(): print(f"  {k}: {v:.6f}")
        print("\nExPreCast final 2024 rollout metrics:")
    ...
    finalize_multi_seed_experiment(
        ...
        best_checkpoint_filename="best_exprecast.pth",
    )
