...
# 淇璺緞锛氬悜涓婅繑鍥炰袱绾э紝纭繚鑳芥壘鍒版牴鐩綍鐨?process.dataset
...
def main():
    ...
    # 鏇存敼淇濆瓨璺緞涓?TimeKAN 涓撳睘鐩綍
    SAVE_DIR = "./model_outcomes/checkpoints_timekan_final"
    ...
    for seed in SEEDS:
        ...
        model = TimeKAN(in_chans=8, hidden_dim=64, seq_len=SEQ_LEN, img_size=(64, 96)).to(device)
        ...
        best_model_path = os.path.join(run_save_dir, "best_timekan.pth")

        print(f"Starting TimeKAN training with seed={seed}...")
        ...
        print("\nTimeKAN final one-step test metrics:")
        for k, v in one_step_metrics.items(): print(f"  {k}: {v:.6f}")
        print("\nTimeKAN final 2024 rollout metrics:")
    ...
    finalize_multi_seed_experiment(
        ...
        best_checkpoint_filename="best_timekan.pth",
    )
